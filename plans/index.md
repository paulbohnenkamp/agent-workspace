# Plans Index

This file is the durable handoff point for active implementation work.

Workflow:

1. Start from `plans/TEMPLATE.md`.
2. Create or update a plan file under `plans/`.
3. Work the plan until the acceptance criteria are met.
4. Mark the plan `done` here.
5. Check off the matching item in `ROADMAP.md`.

## Active Plans

| Roadmap Item | Status | Plan | Notes |
| --- | --- | --- | --- |
| Tighten loader/schema validation for `views/` | planned | [plans/view-metadata-validation.md](./view-metadata-validation.md) | Next slice after the workspace UI milestone. |

## Completed

- [Workspace UI slice milestone](./workspace-ui-slice-milestone.md) - done, outcome summary captured in the plan file
