...
export const SummaryCard: WorkspacePrimitiveComponent = ({ node, interpreted }) => {
  ...
  const artifactFromSelection = selection
    ...
    : undefined;
  const artifact =
    Object.keys(artifactFromBind).length > 0 ? artifactFromBind : asRecord(artifactFromSelection);
  ...
  return (
    <Panel
      title={node.title ?? 'Summary'}
      meta={hasRecord ? <Badge label="Beta" /> : null}
        ...
        {hasRecord ? (
          <div>
            ...
            <p className="workspace-muted">
              {asString(record.role ?? record.type ?? record.status)}
            </p>
          </div>
        ) : null}
        {hasArtifact ? (
          ...
          <p className="workspace-muted">No summary available.</p>
        )}
        {hasRecord ? (
