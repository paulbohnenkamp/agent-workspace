    ...
    <section className="workspace-document">
      {tabs.length > 0 ? (
        <div
          ...
          role="tablist"
          aria-label={asString(artifact.title, 'Document sections')}
        >
          {tabs.map((tab, index) => (
            <span
              className={`workspace-tab ${index === 0 ? 'is-active' : ''}`.trim()}
              key={tab}
              role="tab"
            >
              {tab.split('-').join(' ')}
      ...
      <header className="workspace-document__header">
        <div>
          <h2 className="workspace-document__section-title">
            {asString(artifact.title, 'Document')}
          </h2>
        ...
        {sections.map((section, index) => {
          ...
          return (
            <article
              ...
              key={`${asString(entry.title)}-${index}`}
            >
